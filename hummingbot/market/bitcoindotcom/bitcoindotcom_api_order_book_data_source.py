...
from hummingbot.market.bitcoindotcom.huobi_order_book import HuobiOrderBook

BITCOINDOTCOM_TICKER_URL = "https://api.exchange.bitcoin.com/api/2/public/ticker"
BITCOINDOTCOM_SYMBOLS_URL = "https://api.exchange.bitcoin.com/api/2/public/symbol"
BITCOINDOTCOM_WS_URL = "wss://api.exchange.bitcoin.com/api/2/ws"



class BitcoinDotCommAPIOrderBookDataSource(OrderBookTrackerDataSource):

    MESSAGE_TIMEOUT = 30.0
    PING_TIMEOUT = 10.0

    _bdcaobds_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._bdcaobds_logger is None:
            cls._bdcaobds_logger = logging.getLogger(__name__)
        return cls._bdcaobds_logger
    ...
    async def get_active_exchange_markets(cls) -> pd.DataFrame:
        ...
        async with aiohttp.ClientSession() as client:
            ...
            market_response, exchange_response = await safe_gather(
                client.get(BITCOINDOTCOM_TICKER_URL),
                client.get(BITCOINDOTCOM_SYMBOLS_URL)
            )
            market_response: aiohttp.ClientResponse = market_response
            exchange_response: aiohttp.ClientResponse = exchange_response

            if market_response.status != 200:
                raise IOError(f"Error fetching Bitcoin.com markets information. "
                              f"HTTP status is {market_response.status}.")
            if exchange_response.status != 200:
                raise IOError(f"Error fetching Bitcoin.com exchange information. "
            ...
            trading_pairs: Dict[str, Any] = {
                item["id"]: {attr_name_map[k]: item[k] for k in ["baseCurrency", "quoteCurrency"]} 
            ...
            btc_price: float = float(all_markets.loc["BTCUSD","last"])
            eth_price: float = float(all_markets.loc["ETHUSD","last"])
            bch_price: float = float(all_markets.loc["BCHUSD","last"])
            usd_volume: float = [
                (
                    quoteVolume * btc_price if symbol.endswith("BTC") else
                    quoteVolume * eth_price if symbol.endswith("ETH") else
                    quoteVolume * bch_price if symbol.endswith("BCH") else
                    quoteVolume
                )
...
##To Do - Convert rest of data source methods for Bitcoin.com
"""
    @staticmethod
